...
**What you get back:**
- A short answer, not a report, most of the time. Something like:
  > "This touches `PaymentService.charge()`. It's called from checkout and the retry job — both would be affected. Today a failed charge is final; there's no partial-refund path."
...
**What you get back:**
- For one function or a small question: 3-5 sentences, no headings, no file created.
- For a real feature or flow: a short mental map (a handful of boxes, not the whole system), the rules that must stay true, and the things that are easy to misread on a first pass — usually the single most useful part of the answer.
- For a large, tangled area: a `CODEBASE-UNDERSTANDING.md` or `FLOW-UNDERSTANDING.md` file, only if a file genuinely beats a chat reply.
