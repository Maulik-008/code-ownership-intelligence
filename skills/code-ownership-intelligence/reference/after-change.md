...
Goal: after an AI agent changes code, make sure the human knows what actually happened — not just that it "works now."
...
Find changed files
    ↓
Understand each important change
    ↓
...
Find the changed files from whatever scope was chosen (current diff, a specific commit, a branch) — see the main [SKILL.md](../SKILL.md) scope menu. Then use [research-method.md](research-method.md) for the actual digging, especially **Compare** (how did this work before) and **Challenge** (what if the assumption behind the change is wrong).
...
> **"The new feature works, but did something old quietly stop working?"**
