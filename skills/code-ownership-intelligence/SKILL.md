...
Before any research or writing starts, ask using the terminal's interactive options tool (`AskUserQuestion` with `multiSelect: true` — renders as checkboxes in the Claude Code terminal, not radio buttons), not a plain-text question the human has to type a reply to. **Do not start Step 4 until they've picked.** This is a gate, not a courtesy. Multi-select on purpose: the human can want more than one deliverable from the same research pass (e.g. a Markdown file *and* an interactive walkthrough) instead of being forced to choose only one.

```
question: "What output do you want for this? (pick one or more)"
multiSelect: true
options:
  ...
  - "Quick chat answer only" — a few plain sentences in chat, for something this small/throwaway. Checked alone → that's the entire deliverable. Checked alongside another option → treat it as "give me the quick answer right now, and also build the other thing(s)" — see below.
...
**Handling more than one checked box:** build every format the human checked, from the same research pass — don't ask them to pick again or silently drop one. If "Quick chat answer only" is checked together with something else, it's not a contradiction: give the short chat answer immediately (so they're not left waiting), then keep building the other checked format(s) and deliver them the same way you normally would (file/artifact + pointer). If "Deep explainer" and "Interactive walkthrough" are both checked, build both as separate artifacts — don't try to merge them into one.

The human's picks always win — never substitute a "simpler" format for what they actually checked. Full detail on each format, including the memory-science writing rules, the Deep-explainer skeleton, the quiz-bias guardrails, and the Micro-worlds ground rules: [reference/output-formats.md](reference/output-formats.md).

Once picked, proceed to Step 4 and build everything selected. Every other question this skill asks — ownership checks, explain-it-back, the post-delivery quiz offer, memory-review quizzes — happens only *after* delivery, as normal chat text; they don't need this same gate.

## Step 4 — Research once, then build every picked format
...
**Research once, build each format checked in Step 3.** Don't repeat the research per format — investigate once, then shape that same understanding into every deliverable the human picked. All four formats, their skeletons, length ceilings, and the delivery test ("a tired person skimming for 10 seconds should still walk away knowing the one thing that matters") live in [reference/output-formats.md](reference/output-formats.md) — including:
...
**If more than one format was checked at Step 3:** delivery-agent only ever produces the Markdown file (it has no Artifact tool access) — you, the orchestrator, build any Deep explainer or Interactive walkthrough directly from the same research-agent findings, alongside whatever delivery-agent produces. One research pass, several outputs.
